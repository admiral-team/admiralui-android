...
class UnderlineSliderTabs @JvmOverloads constructor(
    context: Context,
    ...
    private fun populate(viewPager: ViewPager) {
        ...
        for (i in 0 until adapterCount) {
            addView(UnderlineSliderTab(context).apply {
                text = adapter?.getPageTitle(i).toString()

                configureUnderlineTab(onClick, tabConfigurationStrategy, i)
            })
        }
    ...
    private fun populateTabsFromPagerAdapter(
        ...
        if (adapter != null) {
            ...
            for (item in 0 until adapterCount) {
                addView(UnderlineSliderTab(context).apply {
                    configureUnderlineTab(onClick, tabConfigurationStrategy, item)
                })
            }
        }
    ...
    private fun UnderlineSliderTab.configureUnderlineTab(
        onClick: (View) -> Unit,
        tabConfigurationStrategy: TabConfigurationStrategy?,
        i: Int
    ) {
    ...
    fun interface TabConfigurationStrategy {
         ...
         * Typically calls [UnderlineSliderTab.text], but any form of styling can be applied.
         *
         * @param tab The [UnderlineSliderTab] which should be configured to represent the title
         * of the item at the given position in the data set.
         * @param position The position of the item within the adapter's data set.
         */
        fun onConfigureTab(tab: UnderlineSliderTab, position: Int)
    }
}
