package com.admiral.uikit.view
...
open class CircularImageView @JvmOverloads constructor(
    ...
    init {
        parseAttrs(attrs, R.styleable.CircularImageView, defStyleAttr, 0).use { attributes ->
            // Init Background Color
            circleColor = attributes.getColor(R.styleable.CircularImageView_admiralCircleColor, Color.WHITE)
            attributes
                .getColor(R.styleable.CircularImageView_admiralCircleColorStart, 0)
                .also { if (it != 0) circleColorStart = it }
            attributes
                .getColor(R.styleable.CircularImageView_admiralCircleColorEnd, 0)
                .also { if (it != 0) circleColorEnd = it }
            circleColorDirection = attributes.getInteger(
                R.styleable.CircularImageView_admiralCircleColorDirection,
                circleColorDirection.value
            ).toGradientDirection()

            // Init Border
            if (attributes.getBoolean(R.styleable.CircularImageView_admiralBorder, true)) {
                val defaultBorderWidth =
                    DEFAULT_BORDER_WIDTH * resources.displayMetrics.density
                borderWidth = attributes.getDimension(
                    R.styleable.CircularImageView_admiralBorderWidth,
                    defaultBorderWidth
                )
                borderColor =
                    attributes.getColor(R.styleable.CircularImageView_admiralBorderColor, Color.WHITE)
                attributes.getColor(R.styleable.CircularImageView_admiralBorderColorStart, 0)
                    .also { if (it != 0) borderColorStart = it }
                attributes.getColor(R.styleable.CircularImageView_admiralBorderColorEnd, 0)
                    .also { if (it != 0) borderColorEnd = it }
                borderColorDirection = attributes.getInteger(
                    R.styleable.CircularImageView_admiralBorderColorDirection,
                    borderColorDirection.value
                ).toGradientDirection()
            }

            // Init Shadow
            shadowEnable = attributes.getBoolean(R.styleable.CircularImageView_admiralShadow, shadowEnable)
            if (shadowEnable) {
                shadowGravity = attributes.getInteger(
                    R.styleable.CircularImageView_admiralShadowGravity,
                    shadowGravity.value
                ).toShadowGravity()
                val defaultShadowRadius = DEFAULT_SHADOW_RADIUS * resources.displayMetrics.density
                shadowRadius = attributes.getDimension(
                    R.styleable.CircularImageView_admiralShadowRadius,
                    defaultShadowRadius
                )
                shadowColor =
                    attributes.getColor(R.styleable.CircularImageView_admiralShadowColor, shadowColor)
            }
        }
    }
    ...
    override fun setScaleType(scaleType: ScaleType) {
        require(
            ...
            "ScaleType $scaleType not supported. Just ScaleType.CENTER_CROP, " +
                    "ScaleType.CENTER_INSIDE & ScaleType.FIT_CENTER are available for this library."
        }
