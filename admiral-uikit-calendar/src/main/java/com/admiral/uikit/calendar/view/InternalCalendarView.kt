package com.admiral.uikit.calendar.view
...
import com.admiral.uikit.calendar.view.model.DayOwner
import com.admiral.uikit.calendar.view.model.InDateStyle
import com.admiral.uikit.calendar.view.model.MonthConfig
import com.admiral.uikit.calendar.view.model.OutDateStyle
import com.admiral.uikit.calendar.view.model.ScrollMode
import com.admiral.uikit.calendar.view.model.Size
import com.admiral.uikit.calendar.view.ui.CalendarAdapter
import com.admiral.uikit.calendar.view.ui.CalendarLayoutManager
import com.admiral.uikit.calendar.view.ui.CalenderPageSnapHelper
import com.admiral.uikit.calendar.view.ui.DayBinder
import com.admiral.uikit.calendar.view.ui.MonthHeaderFooterBinder
import com.admiral.uikit.calendar.view.ui.MonthScrollListener
import com.admiral.uikit.calendar.view.ui.ViewConfig
import com.admiral.uikit.core.R
import com.admiral.uikit.core.ext.job
...
open class InternalCalendarView : RecyclerView {
     ...
     */
    var maxRowCount = MAX_ROW_COUNT
        set(value) {
            if (!(1..MAX_ROW_COUNT).contains(value)) throw IllegalArgumentException("'maxRowCount' should be between 1 to 6")
            if (field != value) {
                field = value
                updateAdapterMonthConfig()
            }
     ...
     */
    var wrappedPageHeightAnimationDuration = ANIMATION_TIME
    ...
    private fun init(attributeSet: AttributeSet, defStyleAttr: Int, defStyleRes: Int) {
        ...
        context.withStyledAttributes(attributeSet, R.styleable.InternalCalendarView, defStyleAttr, defStyleRes) {
            dayViewResource = getResourceId(R.styleable.InternalCalendarView_cv_dayViewResource, dayViewResource)
            monthHeaderResource =
                getResourceId(R.styleable.InternalCalendarView_cv_monthHeaderResource, monthHeaderResource)
            monthFooterResource =
                getResourceId(R.styleable.InternalCalendarView_cv_monthFooterResource, monthFooterResource)
            orientation = getInt(R.styleable.InternalCalendarView_cv_orientation, orientation)
            scrollMode = ScrollMode.values()[getInt(R.styleable.InternalCalendarView_cv_scrollMode, scrollMode.ordinal)]
            outDateStyle =
                OutDateStyle.values()[getInt(R.styleable.InternalCalendarView_cv_outDateStyle, outDateStyle.ordinal)]
            inDateStyle =
                InDateStyle.values()[getInt(R.styleable.InternalCalendarView_cv_inDateStyle, inDateStyle.ordinal)]
            maxRowCount = getInt(R.styleable.InternalCalendarView_cv_maxRowCount, maxRowCount)
            monthViewClass = getString(R.styleable.InternalCalendarView_cv_monthViewClass)
            hasBoundaries = getBoolean(R.styleable.InternalCalendarView_cv_hasBoundaries, hasBoundaries)
            wrappedPageHeightAnimationDuration = getInt(
                R.styleable.InternalCalendarView_cv_wrappedPageHeightAnimationDuration,
        ...
        if (autoSize && !isInEditMode) {
            ...
            if (widthMode == UNSPECIFIED && heightMode == UNSPECIFIED) {
                throw UnsupportedOperationException(
                    "Cannot calculate the values for day" +
                            " Width/Height with the current configuration."
                )
            }

            // +0.5 => round to the nearest pixel
            val size =
                (((widthSize - (monthPaddingStart + monthPaddingEnd)) / DAYS_OF_WEEK) + NEAREST_PIXEL_DIST).toInt()
    ...
    companion object {
        ...
        private const val DAYS_OF_WEEK = 7f

        private const val NEAREST_PIXEL_DIST = 0.5
